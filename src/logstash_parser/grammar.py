# file: grammar.py
# Defines the grammar elements for logstash conf files
# All ParserElement instances are stored as global variables
...
if_rule = pp.Group(
    pp.Keyword("if")
    + pp.Group(condition + pp.Group(object_start + pp.Group(pp.ZeroOrMore(branch_or_plugin)) + object_stop))
)
if_rule.set_name("if")
...
else_if_rule = pp.Group(
    pp.Combine(pp.Keyword("else") + pp.ZeroOrMore(cs).set_parse_action(pp.replace_with(" ")) + pp.Keyword("if"))
    + pp.Group(condition + pp.Group(object_start + pp.Group(pp.ZeroOrMore(branch_or_plugin)) + object_stop))
)
...
"""
negative_expression = (pp.Literal("!") + parenthesis_start + condition + parenthesis_stop) | (
    pp.Literal("!") + selector
)
...
"""
expression << pp.Group(  # type: ignore[operator]
...
value << (plugin | boolean | bare_word | string | number | array | hashmap)  # type: ignore[operator]
...
rvalue << (string | number | selector | array | method_call | regexp)  # type: ignore[operator]
...
"""
branch_or_plugin << (branch | plugin)  # type: ignore[operator]
...
# Wrap elements with originalTextFor to capture original source text
# This allows us to preserve exact formatting including whitespace
# These wrapped versions are used in build functions to extract original text
# IMPORTANT: We copy() the elements first to avoid inheriting parse actions which would cause recursion

# Core config elements
config_with_source = pp.originalTextFor(config.copy())
plugin_section_with_source = pp.originalTextFor(plugin_section.copy())
plugin_with_source = pp.originalTextFor(plugin.copy())
attribute_with_source = pp.originalTextFor(attribute.copy())
...
array_with_source = pp.originalTextFor(array.copy())
hashmap_with_source = pp.originalTextFor(hashmap.copy())
hash_entry_with_source = pp.originalTextFor(hash_entry.copy())

# Primitive elements
string_with_source = pp.originalTextFor(string.copy())
number_with_source = pp.originalTextFor(number.copy())
bare_word_with_source = pp.originalTextFor(bare_word.copy())
...
compare_expression_with_source = pp.originalTextFor(compare_expression.copy())
in_expression_with_source = pp.originalTextFor(in_expression.copy())
not_in_expression_with_source = pp.originalTextFor(not_in_expression.copy())
